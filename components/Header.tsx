...
const Header: React.FC<HeaderProps> = ({ isDarkMode, toggleTheme }) => {
  ...
  // Auto-fold timer logic ONLY active on the Drive page
  useEffect(() => {
    if (!isDrivePage) {
    ...
    const resetInactivityTimer = () => {
      ...
      inactivityTimerRef.current = setTimeout(() => {
        setIsDriveExpanded(false);
      }, 3500); // Folds into circle after 3.5s of inactivity
    };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 bg-transparent transition-all duration-300 pt-4 sm:pt-6 px-4 sm:px-10 md:px-12 lg:px-16 xl:px-20 pointer-events-none">
      
      {/* DRIVE PAGE FOLDED STATE (CIRCULAR FAVICON BUTTON) */}
      {isDrivePage && !isDriveExpanded ? (
        <div className="flex justify-center pointer-events-auto">
          <button
            onClick={() => setIsDriveExpanded(true)}
            className="w-12 h-12 rounded-full bg-black/80 dark:bg-white/10 backdrop-blur-md border border-white/20 shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 group"
            title="Open Navigation"
            aria-label="Open Navigation Menu"
          >
            <span className="text-theme-red font-black text-lg group-hover:rotate-12 transition-transform">
              AK
            </span>
          </button>
        </div>
      ) : (
        /* STANDARD FLOATING GLASS PILL (ALWAYS ON OTHER PAGES, FOLDS ON DRIVE PAGE) */
        <div className="max-w-[1440px] mx-auto h-16 md:h-20 flex items-center justify-between px-4 sm:px-8 md:px-10 lg:px-16 xl:px-20 
                        ...
                        rounded-full shadow-lg transition-all duration-500 pointer-events-auto">
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="lg:hidden fixed top-24 left-4 right-4 z-30 bg-white/90 dark:bg-dark-bg/90 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl overflow-hidden pointer-events-auto">
